<template>
  <div style="height: inherit">
    <global-layout v-if="!nonGlobal" style="height: inherit" @dynamicRouterShow="dynamicRouterShow">
      ...
      <a-tabs
        ...
        style="height:52px;position: relative;z-index: 100"
        :hide-add="true"
        type="editable-card"
      ...
      <template>
        ...
        <keep-alive v-if="multipage" :exclude="['Hbgzt']">
      ...
      <!--      <div id="content" class="app-view-box" />-->
...
const indexKey = '/dashboard/analysis_废除'// 根据indexKey判断的逻辑全部取消
const nonGlobal = ['/home']
export default {
  name: 'TabLayout',
  components: {
    GlobalLayout,
  ...
  computed: {
    nonGlobal() {
      if (nonGlobal.indexOf(this.$route.path) !== -1) {
        return true
      } else {
        return false
      }
  ...
  watch: {
    '$route': function(newRoute) {
      ...
      if (!this.multipage) {
        ...
        if (this.linkList.find(i => i.indexOf(newRoute.path) > -1)) {
          this.linkList = this.linkList.filter(i => i !== newRoute.path)
          this.pageList = this.pageList.filter(i => i.path !== newRoute.path)
        }
  ...
  methods: {
    ...
    changeTabTitle(title, fullPath = '') {
      if (title) {
        const currentRoute = this.pageList.find((r) => r.fullPath === (fullPath || this.$route.fullPath))
    ...
    remove(key) {
      ...
      if (removeRoute && removeRoute[0]) {
        const componentName = removeRoute[0].name
        console.log('key: ', key)
    ...
    /* update_end author:wuxianquan date:20190828 for: 关闭当前tab页，供子页面调用->望菜单能配置外链，直接弹出新页面而不是嵌入iframe #428 */
    closeOthers(pageKey) {
      this.linkList = this.linkList.filter(i => i === pageKey)
      this.pageList = this.pageList.filter(i => i.fullPath === pageKey)
