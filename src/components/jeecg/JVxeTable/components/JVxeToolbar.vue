<template>
  <div :class="boxClass">
    ...
    <div class="j-vxe-tool-button div" :size="btnSize">
      ...
      <!-- <a-button v-if="showdUplicate" @click="trigger('uplicate')">复制行</a-button> -->
      <a-button v-if="showAdd" icon="plus" :disabled="disabled" type="primary" @click="trigger('add')">新增</a-button>
      <a-button v-if="showSave" icon="save" :disabled="disabled" @click="trigger('save')">保存</a-button>
...
export default {
  ...
  computed: {
    ...
    btns() {
      const { btn, btns } = this.toolbarConfig
      const arr = btn || btns || ['add', 'remove', 'clearSelection']
    ...
    showdUplicate() {
      return this.btns.includes('uplicate')
    },
    showClearSelection() {
