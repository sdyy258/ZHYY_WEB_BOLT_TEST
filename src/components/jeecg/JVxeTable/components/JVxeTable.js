      ...
      async handler() {
        ...
        this.dataSource.forEach((data, idx) => {
          ...
          if (this.innerLinkageConfig != null) {
            for (const configItem of this.innerLinkageConfig.values()) {
      ...
      handler() {
        if (Array.isArray(this.linkageConfig) && this.linkageConfig.length > 0) {
          ...
          const getLcKeys = (key, arr) => {
            const col = this.innerColumns.find(col => col.key === key)
            if (col) {
              arr.push(col.key)
              if (col.linkageKey) {
      ...
      xTable.tableFullColumn.forEach(column => {
        ...
        if (col.$type === JVXETypes.select && this.innerLinkageConfig != null) {
          ...
          if (this.innerLinkageConfig.has(col.key)) {
            const configItem = this.innerLinkageConfig.get(col.key)
