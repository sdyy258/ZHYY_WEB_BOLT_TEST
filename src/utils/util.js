...
export function generateIndexRouter(data) {
  let redirectPath = ''
  const noHiddenData = data.filter(item => item.hidden !== true)
  if (noHiddenData.length > 0) {
    redirectPath = noHiddenData[0].path
    if (noHiddenData[0].children && noHiddenData[0].children.length > 0) {
      const childData = noHiddenData[0].children
      const noHiddenChildData = childData.filter(item => item.hidden !== true)
      if (noHiddenChildData.length > 0) {
        redirectPath = noHiddenChildData[0].path
      }
    }
  }
  const indexRouter = [{
