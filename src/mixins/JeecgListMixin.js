...
export const JeecgListMixin = {
  ...
  methods: {
      ...
      getAction(this.url.list, params).then((res) => {
        if (res.success) {
          ...
          if (res.result.total) {
            ...
            // 解决后端分页查询是由于末尾添加数据total比总数据少，此时前端分页pageSize应传大值
            if (res.result.records.length > res.result.total) {
    ...
    copyDateil: function(record) {
      this.$refs.modalForm.title = '复制'
      this.$refs.modalForm.showModal(record)
      this.$refs.modalForm.disableSubmit = false
    },
    ...
    handleExportXls_Url(fileName, exportUrl) {
      if (!fileName || typeof fileName !== 'string') {
        fileName = '导出文件'
      }
      const param = this.getQueryParams()
