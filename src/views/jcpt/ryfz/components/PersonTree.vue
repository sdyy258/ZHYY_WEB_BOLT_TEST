<template>
  <a-card :loading="cardLoading" :bordered="false" style="height: 100%;width: 22%;overflow-y: auto">
    <a-spin :spinning="loading">
      <a-input-search style="width:100%;margin-top: 10px" placeholder="输入机构名称查询..." enter-button @search="handleSearch" />
      <a-tree
        show-line
        check-strictly
        ...
        :replace-fields="{title:'departName',key:'id',children:'children'}"
...
<script>
import { queryDepartTreeListNew, searchByKeywords } from '@/api/api'
...
export default {
  ...
  methods: {
    ...
    queryTreeData(keyword) {
      this.commonRequestThen(queryDepartTreeListNew({
    ...
    handleTreeSelect(selectedKeys, event) {
      if (selectedKeys.length > 0 && this.selectedKeys[0] !== selectedKeys[0]) {
        ...
        this.emitSelect(event.node.dataRef)
      }
    },
