...
export default {
  ...
  methods: {
    showModal(val, name, questionId, fixedCategoryId) {
      this.visible = true
      this.confirmLoading = false
      const questionType = this.$utils.getDictText(dataType, name, 'text', 'value')
      this.model = {
        isRepeatExtract: '1',
        questionType: questionType ? parseInt(questionType) : ''
      }
      this.categoryId = []
      this.index = val
      ...
      // else {
      //   this.categoryId.push(this.questionList[0].value)
      // }
      // this.getQuestionNum()
    ...
    handleOk() {
      ...
      if (this.model.questionType === 7) {
        if (this.model.num > this.extractList.analysis) {
          this.$message.warning('抽题数量应小于试题数量')
          return false
        }
      }
      if (this.model.questionType === 8) {
        if (this.model.num > this.extractList.essay) {
          this.$message.warning('抽题数量应小于试题数量')
          return false
        }
      }
          ...
          getAction('/tQuestion/tQuestion/paperExtractQuestions', that.model)
            .then(res => {
              if (res.success) {
                ...
                that.$emit('ok', result, this.index, this.model.isRepeatExtract, this.categoryId)
