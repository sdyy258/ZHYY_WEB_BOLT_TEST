        ...
        <div class="whole-top flex">
          <div class="left-type flex1">
            ...
            <table border="0" width="100%" height="260px">
              ...
              <tr class="type-table1">
                ...
                <td>
                  <a-input-number
                    v-model="examPaperRule.shortAnswer"
                    :precision="0"
                    :min="0"
                    :max="extractList.shortAnswer"
                    @change="fillChange($event, 'shortAnswer', 'shortAnswerScore')"
                  />
                </td>
                <td>
                  <a-input-number
                    v-model="examPaperRule.shortAnswerScore"
                    :precision="1"
                    :min="0.5"
                    @change="scoreChange($event, 'shortAnswer', 'shortAnswerScore')"
                  />
                </td>
              ...
              <tr class="type-table1">
                ...
                <td>
                  <a-input-number
                    v-model="examPaperRule.calculate"
                    :precision="0"
                    :min="0"
                    :max="extractList.calculate"
                    @change="fillChange($event, 'calculate', 'calculateScore')"
                  />
                </td>
                <td>
                  <a-input-number
                    v-model="examPaperRule.calculateScore"
                    :precision="1"
                    :min="0.5"
                    @change="scoreChange($event, 'calculate', 'calculateScore')"
                  />
                </td>
              ...
              <tr class="type-table1">
                ...
                <td>
                  <a-input-number
                    v-model="examPaperRule.analysis"
                    :precision="0"
                    :min="0"
                    :max="extractList.analysis"
                    @change="fillChange($event, 'analysis', 'analysisScore')"
                  />
                </td>
                <td>
                  <a-input-number
                    v-model="examPaperRule.analysisScore"
                    :precision="1"
                    :min="0.5"
                    @change="scoreChange($event, 'analysis', 'analysisScore')"
                  />
                </td>
                <td>{{ analysisScore1 || '0' }}</td>
              </tr>
              <tr class="type-table1">
                <td>论述题</td>
                <td>
                  <a-input-number
                    v-model="examPaperRule.essay"
                    :precision="0"
                    :max="extractList.essay"
                    :min="0"
                    @change="fillChange($event, 'essay', 'essayScore')"
                  />
                </td>
                <td>
                  <a-input-number
                    v-model="examPaperRule.essayScore"
                    :precision="1"
                    :min="0.5"
                    @change="scoreChange($event, 'essay', 'essayScore')"
                  />
                </td>
                <td>{{ essayScore1 || '0' }}</td>
              </tr>
            ...
            <!-- <div style="color:red;margin-top: 15px;">
              最多可选：单选题(10道)，多选题(10道)，判断题(10道)，填空题(10道)，简答题(10道)，计算题(10道)，分析题(10道)，论述题(10道)。
            </div> -->
            <div style="color:red;">
              备注：每题分数必须为整数且不得小于0.5分；满分小于等于100分。
            </div>
          </div>
          <div class="right-sample flex1">
            ...
            <div class="sample-all" style="height: auto;">
              ...
              <div
                v-if="
                  $store.getters.userInfo.roles.indexOf('admin') > -1 ||
                    $store.getters.userInfo.roles.indexOf('stadmin') > -1 ||
                    role.indexOf('dsgly') == -1
                "
                class="sample-foot"
              >
                <div class="foot-text">
...
export default {
  ...
  methods: {
    ...
    fillChange(e, num, val) {
      ...
      if (num == 'shortAnswer' && e > this.extractList.shortAnswer) {
        this.$message.warning('简答题所选数量超过题库数量')
      }
      if (num == 'calculate' && e > this.extractList.calculate) {
        this.$message.warning('计算题所选数量超过题库数量')
      }
      if (num == 'analysis' && e > this.extractList.analysis) {
        this.$message.warning('分析题所选数量超过题库数量')
      }
      if (num == 'essay' && e > this.extractList.essay) {
        this.$message.warning('论述题所选数量超过题库数量')
      }
    ...
    countTotal() {
      ...
      const num4 = Number(this.analysisScore1 || '0') + Number(this.essayScore1 || '0')
      this.model.totalScore = Number(num1 || '0') + Number(num2 || '0') + Number(num3 || '0') + Number(num4 || '0')
      // this.model.totalScore = Number(num1 || '0') + Number(num2 || '0')
      const num5 = multiplication(this.model.totalScore, 0.6)
      this.model.passScore = Math.round(num5)
    },
    // 获取试题分类树
    getQuestion() {
    ...
    handleOk() {
      if (this.model.totalScore > 100) {
        this.$message.warning('满分必须小于等于100分')
        return false
      }
