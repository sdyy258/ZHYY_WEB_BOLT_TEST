<template>
  <div>
    <div v-if="dataSource.length>0">
      <div class="item">
        <div v-for="(item,index) in infoList" :key="index" class="item-exam animate">
          <div class="item-img">
            <img src="../picture.jpg" alt>
          </div>
          <div class="item-title" :title="item.prop1">{{ item.prop1 }}</div>
          <div class="item-row item-end-time">
            <span>错题数量：{{ item.prop2 }}</span>
          </div>
          <div class="item-row item-end-time">
            <span>考试成绩：{{ item.prop3 }}</span>
          </div>
          <div class="item-footer">
            <a-button type="primary" @click="WrongQuestionView">查看</a-button>
          </div>
        </div>
      </div>
    </div>
