  ...
  <a-card :bordered="false">
    ...
    <div>
      ...
      <a
        ...
        :class="{ gary: !disabledClick, blue: disabledClick }"
        @click="secondFileClick('', 1)"
      >{{ secondfileName }}</a>
    ...
    >
      ...
      <span slot="filePath" slot-scope="text, record">
        <a-icon
          type="file-pdf"
          theme="filled"
          style="color:#3C82FD;font-size: 18px;margin-right: 5px;cursor: default;"
        />
      ...
      <span slot="operation" slot-scope="text, record" class="table-action">
        ...
        <template v-if="!disabledClick && !secondFileShow">
          ...
          <template v-else>
            <a-popconfirm
              v-if="$store.getters.userInfo.roles.indexOf('dsgly') == -1"
              title="确定删除吗?"
              @confirm="() => handleDelete(record.categoryId, 1)"
            >
              <a>删除</a>
            </a-popconfirm>
          </template>
        ...
        <template v-if="!disabledClick && secondFileShow">
          ...
          <template v-else>
            <a-popconfirm
              v-if="$store.getters.userInfo.roles.indexOf('dsgly') == -1"
              title="确定删除吗?"
              @confirm="() => handleDelete(record.categoryId, 2)"
            >
              <a>删除</a>
            </a-popconfirm>
          </template>
        ...
        <template v-if="disabledClick">
          ...
          <template v-else>
            <a-popconfirm
              v-if="$store.getters.userInfo.roles.indexOf('dsgly') == -1"
              title="确定删除吗?"
              @confirm="() => handleDeleteFile(record.id)"
            >
              <a>删除</a>
            </a-popconfirm>
          </template>
...
const columns1 = columns.filter(
  item =>
    item.dataIndex !== 'standardName' &&
    item.dataIndex !== 'fileSize' &&
    item.dataIndex !== 'filePath' &&
    item.dataIndex !== 'isAudit_dictText' &&
    item.dataIndex !== 'backReason'
)
export default {
  name: 'Index',
  ...
  methods: {
    ...
    secondFileClick(record, current) {
      ...
      var params = this.getQueryParams() // 查询条件
      params.pid = this.pid
      getAction(this.url.childrenList, params)
        .then(res => {
          if (res.success) {
            this.dataSource = res.result.records
    ...
    folderClick(record, current, tabCurrent) {
      ...
      getAction(this.url.fileList, params)
        .then(res => {
          if (res.success) {
            this.dataSource = res.result.records
    ...
    handleEdit(record) {
      this.loading = true
      postAction(this.url.edit, { author: record.author, id: record.id })
        .then(res => {
          if (res.success) {
    ...
    handleDeleteFile(id, index) {
      var that = this
      that.loading = true
      deleteAction(that.url.deleteFile, { id: id })
        .then(res => {
          if (res.success) {
            // 重新计算分页问题
            that.reCalculatePage(1)
