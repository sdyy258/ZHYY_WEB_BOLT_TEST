  ...
  <a-card :bordered="false">
    <a-form layout="inline" class="search-form" @keyup.enter.native="searchQuery">
      ...
      <a-form-item label="课程状态">
        <j-dict-select-tag
          v-model.trim="queryParam.courseStatus"
          trigger-change
          dict-code="course_status"
      ...
      <a-form-item label="课件数量">
        <a-input-number
          v-model.trim="queryParam.coursewareNumber"
          :min="0"
          :precision="0"
          placeholder="请输入课件数量"
          style="width: 100%;"
    ...
    <modal-form ref="modalForm" @close="loadData" />
  </a-card>
</template>
...
export default {
  ...
  methods: {
    ...
    connectParams(params) {
      if (this.$store.getters.userInfo.roles.indexOf('admin') > -1 || this.$store.getters.userInfo.roles.indexOf('stadmin') > -1) {
        return false
      } else {
        if (this.$store.getters.userInfo.roles.indexOf('dsgly') > -1) {
          params.isDsUpload = 1
        }
      }
    },
    ...
    handleEdit(record) {
      getAction(this.url.queryById, { id: record.id }).then((res) => {
        if (res.success) {
          this.$refs.modalForm.result = res.result
          this.$refs.modalForm.recordId = record.id
