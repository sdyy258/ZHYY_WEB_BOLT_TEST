          ...
          <a
            v-if="
              record.questionType == 1 ||
                record.questionType == 2 ||
                record.questionType == 3 ||
                record.questionType == 4
            "
            @click="$refs.practiceModal.showModal(record)"
          >练习</a>
<!--          <template v-if="$store.getters.userInfo.departInfo.orgLevel == 1">-->
          <template>
            <a v-if="record.isReview != 1" @click="$refs.examineModal.showModal(record)">审核</a>
          </template>
        </span>
...
<script>
import { dataType, difficultyLevel, columns } from './dataSources'
...
export default {
  ...
  name: 'stgl-tkgl',
  components: { QuestionTree, JTreeSelect, PracticeModal, ExamineModal },
