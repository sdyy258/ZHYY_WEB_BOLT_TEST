...
import { canvasToBlob } from '@/utils/custom/utils'
export default {
  data() {
    return {
      visible: false,
  ...
  methods: {
    showModal(record) {
      ...
      // this.trainNameShow = false
      getAction(this.url.list, { id: record.id }).then(async(res) => {
        if (res.success) {
          ...
          // 下发年
          this.model.creatYear = res.result.issuanceTime && res.result.issuanceTime.split(' ')[0].split('-')[0] || ''
          // 下发月
          this.model.creatMouth = res.result.issuanceTime && res.result.issuanceTime.split(' ')[0].split('-')[1] || ''
          // 下发日
          this.model.creatDay = res.result.issuanceTime && res.result.issuanceTime.split(' ')[0].split('-')[2] || ''
          // if (this.model.trainName.length < 21) {
          //   this.trainNameShow = true
          // }
          this.visible = true
        } else {
    ...
    downloadImage() {
      ...
      html2canvas(element, {
        ...
        this.$utils.canvasToBlob(canvas, '培训证书')
